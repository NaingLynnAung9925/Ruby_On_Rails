...
class AuthsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create auth" do
    assert_difference('Auth.count') do
      post auths_url, params: { auth: { email: @auth.email, name: @auth.name, password: 'secret', password_confirmation: 'secret' } }
    end
  ...
  test "should update auth" do
    patch auth_url(@auth), params: { auth: { email: @auth.email, name: @auth.name, password: 'secret', password_confirmation: 'secret' } }
